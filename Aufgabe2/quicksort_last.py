...
def quicksort_last(list, start, end):
    comparisons_total = 0
    swaps_total = 0

    if start < end:
        pivot_index, comparisons, swaps = partition_last(list, start, end)
        comparisons_total += comparisons
        swaps_total += swaps

        left_comparisons, left_swaps = quicksort_last(list, start, pivot_index - 1)
        comparisons_total += left_comparisons
        swaps_total += left_swaps

        right_comparisons, right_swaps = quicksort_last(list, pivot_index + 1, end)
        comparisons_total += right_comparisons
        swaps_total += right_swaps
        
    return comparisons_total, swaps_total

def partition_last(list, start, end):
...
if __name__ == "__main__":
    ...
    for name, test_list in test_cases.items():
        comparisons, swaps = quicksort_last(test_list, 0, len(test_list) - 1)
    ...
    comparisons, swaps = quicksort_last(arr1, 0, len(arr1) - 1)
